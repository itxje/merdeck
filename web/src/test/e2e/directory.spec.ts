...
test('excluded-only continuations remain navigable, file types list below a folder, with deferred Markdown states', async ({ page, audit }) => {
  ...
  try {
    ...
    await expect(page.getByRole('article', { name: 'Markdown document', exact: true })).toContainText('No diagrams')
...
test('narrow source theme changes preserve fitted preview geometry and directory keyboard navigation', async ({ page }, info) => {
  ...
  const inside = await page.locator('.document-diagram.selected .diagram-graphic svg').evaluate((svg) => {
