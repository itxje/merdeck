...
const samples = process.env.MERDECK_TEST_PERFORMANCE_SAMPLES === '1' ? 1 : 4
if (!root)
...
async function openSample(page: Parameters<typeof login>[0], name: string, visible: 'text' | 'diagram', completeText?: string): Promise<Sample> {
  ...
  if (completeText) {
    const paragraph = article.locator('p').last()
    await expect(article.locator('[data-document-text-complete]')).toBeVisible()
    await expect(paragraph).toHaveText(completeText)
  }
...
test('production Chromium measures worker document readiness and viewport-diagram deferral', async ({ page }) => {
  ...
  // The diagnostic is deliberately bounded: a single 1 MiB document must finish
  // complete decoded-text materialization inside this window, not merely show its heading.
  test.setTimeout(samples === 1 ? 30000 : 180000)
  await mkdir(join(process.cwd(), '..', 'tmp'), { recursive: true })
  const id = randomUUID()
  const largeNames = Array.from({ length: samples }, (_, index) => `markdown-performance-large-${id}-${index}.md`)
  const diagramsNames = Array.from({ length: samples }, (_, index) => `markdown-performance-diagrams-${id}-${index}.md`)
  ...
  const prefix = '# Large document\n\n'
  const paragraph = (`Worker &amp; parser. ${body.repeat(Math.ceil(oneMiB / body.length))}`).slice(0, oneMiB - prefix.length - tail.length) + tail
  const large = `${prefix}${paragraph}`
  const decodedParagraph = paragraph.replace('&amp;', '&')
  ...
  try {
    ...
    for (const name of largeNames) {
      largeSamples.push(await openSample(page, name, 'text', decodedParagraph))
      expect(largeSamples.at(-1)!.stages.some(stage => stage.name === 'worker-fallback' || stage.name === 'worker-runtime-error')).toBe(false)
    }
