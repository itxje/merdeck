...
async function textContrast(page: Page, subject: Locator, behind: Locator = subject) {
  ...
  return page.evaluate(({ subjectNode, behindNode }) => {
    ...
    // Text is composited over its own background too, so a translucent ink measures as the pixel it
    // paints rather than as the colour it was declared in.
    const [light, dark] = [luminance([getComputedStyle(subjectNode).color, ...paintedBackground(subjectNode)]), luminance(paintedBackground(behindNode))].sort((first, second) => second - first)
