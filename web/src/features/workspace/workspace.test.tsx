...
it('keeps Markdown in the document reader without a redundant mode toolbar', async () => {
  ...
  expect(screen.queryByRole('tablist', { name: 'Markdown view' })).toBeNull()
  expect(screen.getByLabelText('Mermaid source', { exact: true })).toHaveValue('flowchart LR\nA-->B')
  ...
  await waitFor(() => expect(screen.getByRole('heading', { name: 'First' })).toBeVisible())
  expect(screen.queryByRole('tablist', { name: 'Markdown view' })).toBeNull()
  view.unmount()
  client.clear()
})
...
it('docks the AI file editor open by default and keeps it closed once dismissed', async () => {
  localStorage.clear()
  vi.spyOn(api, 'session').mockResolvedValue({ authenticated: true, access: 'open', version: '0.0.0-test', pollIntervalMs: 30000, maxSourceBytes: 1048576, storage: { writable: true, identity: 'stable', filesystemType: 'test', supportedFilesystem: 'linux-overlayfs' } })
  vi.spyOn(api, 'directory').mockResolvedValue({ path: '', parent: null, entries: [], revision: 'a'.repeat(64), complete: true, nextCursor: null, expiresAt: null, stoppedBy: null, visited: 0, excluded: 0, limit: 100, maxPathDepth: 64, pollIntervalMs: 30000 })
  vi.spyOn(api, 'directoryRevision').mockResolvedValue({ path: '', revision: 'a'.repeat(64), maxPathDepth: 64, pollIntervalMs: 30000 })
  const client = createQueryClient()
  const view = render(<QueryClientProvider client={client}><ThemeProvider><Workspace path="" block={0} navigate={vi.fn()} /></ThemeProvider></QueryClientProvider>)
  const toggle = await screen.findByRole('button', { name: 'Open AI file editor' })
  expect(toggle).toHaveAttribute('aria-pressed', 'true')
  await userEvent.setup().click(toggle)
  expect(toggle).toHaveAttribute('aria-pressed', 'false')
  expect(localStorage.getItem('merdeck-agent-open')).toBe('false')
  view.unmount()
  // A dismissed panel stays dismissed for the next visit.
  const revisit = render(<QueryClientProvider client={client}><ThemeProvider><Workspace path="" block={0} navigate={vi.fn()} /></ThemeProvider></QueryClientProvider>)
  expect(await screen.findByRole('button', { name: 'Open AI file editor' })).toHaveAttribute('aria-pressed', 'false')
  revisit.unmount()
  localStorage.clear()
  client.clear()
})
