...
type Node = { type: string, value?: string, lang?: string | null, url?: string, alt?: string | null, depth?: number, checked?: boolean | null, ordered?: boolean, align?: (string | null)[], children?: Node[], position?: { start: { line: number }, end: { line: number } } }
...
function InlineDiagram({ source, selected, onSelect }: { source: string, selected: boolean, onSelect: () => void }) {
  ...
  React.useEffect(() => {
    let current = true
    void renderDiagram(source, () => current).then(svg => { if (current && svg) setResult({ source, svg }) }, error => { if (current) setResult({ source, error: error instanceof Error ? error.message : 'Unable to render diagram.' }) })
    return () => { current = false }
  }, [source])
  const visible = result?.source === source ? result : null
  return <figure className={`document-diagram${selected ? ' selected' : ''}`} onClick={onSelect}>
...
function resolveProjectLink(path: string, url: string): string | null {
  if (!/^(?:[^:/?#]+\/)*[^/?#]+\.(?:md|mmd|mermaid)(?:#.*)?$/i.test(url)) return null
...
export function DocumentView({ text, path, blocks, sources, selected, onSelect, onOpenFile }: { text: string, path: string, blocks: DiagramBlock[], sources: string[], selected: number, onSelect: (index: number) => void, onOpenFile: (path: string) => void }) {
  ...
  const render = (node: Node, key: string): React.ReactNode => {
    ...
    if (node.type === 'heading') return React.createElement(`h${Math.min(6, node.depth ?? 1)}`, { key }, children)
    ...
    if (node.type === 'listItem') return <li key={key}>{node.checked !== null && node.checked !== undefined && <input type="checkbox" checked={node.checked} readOnly tabIndex={-1} />}{children}</li>
    if (node.type === 'image') return <span key={key} className="document-image">Image: {node.alt || 'image'} ({node.url})</span>
    if (node.type === 'link') {
      const project = node.url ? resolveProjectLink(path, node.url) : null
      if (project) return <button key={key} type="button" className="document-link" onClick={() => onOpenFile(project)}>{children}</button>
      if (node.url && /^(https?:|mailto:)/i.test(node.url)) return <a key={key} href={node.url} target="_blank" rel="noopener noreferrer">{children}</a>
      return <React.Fragment key={key}>{children}</React.Fragment>
    }
    if (node.type === 'code') {
      const found = node.position ? matched.get(`${node.position.start.line}:${node.position.end.line}`) : undefined
      if (placementOk && node.lang === 'mermaid' && found !== undefined) return <InlineDiagram key={key} source={sources[found] ?? blocks[found]!.source} selected={selected === found} onSelect={() => onSelect(found)} />
    ...
    if (node.type === 'footnoteDefinition') return <aside key={key}>{children}</aside>
    return <React.Fragment key={key}>{children}</React.Fragment>
  }
  return <article className="document-view" aria-label="Markdown document">{!placementOk && <p className="document-mismatch" role="alert">Diagram placement could not be verified; Mermaid fences are shown as code.</p>}{tree.children.map((node, index) => render(node, `root-${index}`))}</article>
}
