...
test('configured service exports preserve whole-file contracts for transport consumers', async () => {
  ...
  try {
    ...
    expect(document.text).toBe(source.slice(1))
    ...
    expect(saved.text).toBe(expected.slice(1))
...
test('ordinary README contexts retain multiple byte-exact independently saved diagrams', async () => {
  ...
  try {
    ...
    expect(original.text).toBe((prefix + first + middle + second).slice(1))
    ...
    expect(savedFirst.text).toBe((prefix + newFirst + middle + second).slice(1))
    ...
    expect(savedSecond.text).toBe(expected.slice(1))
