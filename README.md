...
- `GET /api/diagrams/search?path=docs&query=relay` searches the names below one folder, including every subfolder, and returns up to 200 matching visible entries with `complete`, `stoppedBy` (`matches`, `visits` or `time`), `visited` and `skipped`. It reads names only, never file contents, applies the same exclusions and path checks as a directory page, and stops at 20,000 names or before the request deadline. `query` is 1 to 200 characters of text without control characters, matched case-insensitively against the path below the folder. An incomplete result never proves that a file is absent.
...
Download `merdeck.tar.gz` and `SHA256SUMS` from [v0.9.1](https://github.com/itxje/merdeck/releases/tag/v0.9.1). The earlier v0.9.0 tag remains unpublished after failed release checks; it was not moved. Both names stay the same for every release, so `https://github.com/itxje/merdeck/releases/latest/download/merdeck.tar.gz` always names the current one. The directory-navigation archive requires exactly [Bun](https://bun.sh) **1.4.2**, Linux **little-endian LP64 x64 or arm64**, mounted procfs and dynamically linked **glibc >= 2.30** (`libc.so.6`). Other runtimes, architectures and libc implementations are not covered by this support contract. In an empty download directory, use an owned tmux session for persistent execution:
