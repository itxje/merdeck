...
Select `bun-linux-x64` for the required release target; cross-compilation alone is not execution proof. Use an empty dist/release directory for standalone compile commands; prior output is retained rather than silently overwritten. `check:ci` owns a unique staging directory and retains prior complete results under tmp/. Public attachments are the raw executable and SHA256SUMS; the internal manifest is only verification metadata. There is no separate web asset archive and no runtime-required fallback.
...
After a real release exists, download its `merdeck-VERSION-linux-x64` and `SHA256SUMS` from that matching release. In the download directory, use an owned tmux session for persistent execution; choose the matching tag/version and target before running:

```bash
sha256sum --check SHA256SUMS
chmod +x merdeck-VERSION-linux-x64
./merdeck-VERSION-linux-x64 --version
