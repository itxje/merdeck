...
Below the search field, **All**, **.mmd** and **.md** choose file types within the loaded window. Search also applies only to loaded files; folders always remain navigable. The kind choice is remembered in this browser and applies to the file drawer. Retained drafts stay reachable when filtering, paging or browsing elsewhere. Directory failures do not establish that the selected file was deleted; its own revision check controls saving.
...
Download `merdeck.tar.gz` and `SHA256SUMS` from the published v0.8.4 release. The next expected release, v0.9.0, remains pending final validation and publication. Both names stay the same for every release, so `https://github.com/itxje/merdeck/releases/latest/download/merdeck.tar.gz` always names the current one. For this directory-navigation candidate, the archive requires exactly [Bun](https://bun.sh) **1.4.2**, Linux **little-endian LP64 x64 or arm64**, mounted procfs and dynamically linked **glibc >= 2.30** (`libc.so.6`). Other runtimes, architectures and libc implementations are not covered by this support contract. In an empty download directory, use an owned tmux session for persistent execution:
