...
The AI file editor is opt-in and requires token access. Set `MERDECK_CODEX_PATH` and/or `MERDECK_CLAUDE_PATH` to an absolute executable path outside `MERDECK_ROOT`, then restart the service. Merdeck uses the CLI's existing file-based login and never accepts provider API keys from the browser. Enabling a provider permits it to read project context and send that context and prompts to its remote service. Provider edits are direct external writes beneath `MERDECK_ROOT`; they do not use Merdeck's expected-version save transaction and are not guaranteed to request approval first. Leave both paths unset to keep the feature disabled.
...
With token access, open **AI file editor** from the header, choose an engine and select one of the models reported by that engine's initialized CLI protocol. The bounded catalogue is cached for the service lifetime; if discovery fails, the only choice is **Provider default**. Engine and model stay fixed for that conversation. A conversation is memory-only and belongs to the current signed-in session and origin. Send is disabled while any browser draft is dirty or saving, and application saves and file operations are locked while a turn is active. Provider file-change events and a faster selected-file revision check refresh a clean open document; completion forces a final document and directory reconciliation. Use **Stop** to cancel a turn. When the provider requests file-access, file-change or command approval, review it before choosing **Approve** or **Deny**.

This is direct CLI editing, not an atomic Merdeck save. The provider can edit files beneath the configured project according to its own sandbox, and another local process can still race those writes. The Codex adapter requests workspace-write with tool-network access disabled; the Claude Code adapter exposes only `Read`, `Edit`, `Write`, `Glob` and `Grep`, with commands and web tools unavailable. Provider output is rendered as text, provider IDs and raw stderr stay server-side, and logout, expiry, Stop or service shutdown reaps the child process. Do not enable the feature for a project whose content may not be sent to the selected provider.
...
AI routes are available only to token-authenticated sessions when at least one provider executable is configured. GET `/api/agents/capabilities` returns provider labels and validated, bounded model metadata discovered from each CLI. POST `/api/agents/conversations` requires an advertised engine/model pair; `/turns`, `/approvals/:approvalId` and `/cancel` use the existing Origin/CSRF boundary; GET `/events` is a bounded authenticated SSE stream. Bodies are strict and bounded. Browser-visible IDs are opaque, and no route accepts an executable, working directory, command, provider credential or provider session ID.
