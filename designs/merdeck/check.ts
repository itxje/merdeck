...
async function main() {
  ...
  assert.deepEqual(assets.map(asset => asset.path).sort(), ['Directory-navigation.html', 'Merdeck.html', 'Mobile-file-drawer.html', 'brand.html'])
  const expectedStatus: Record<string, string> = { 'Directory-navigation.html': 'needs-review', 'Merdeck.html': 'needs-review', 'Mobile-file-drawer.html': 'needs-review', 'brand.html': 'approved' }
  ...
  const mobileDrawer = await readFile(resolve(directory, 'Mobile-file-drawer.html'), 'utf8')
  const mobileEnvelope = mobileDrawer.replace(/<script>[\s\S]*<\/script>/, '<script></script>')
  assert.equal((mobileEnvelope.match(/<script>/g) ?? []).length, 1, 'Mobile drawer script count')
  assert.equal((mobileDrawer.match(/<\/script>/g) ?? []).length, 1, 'Mobile drawer closing-script count')
  assert.equal((mobileEnvelope.match(/<style>/g) ?? []).length, 1, 'Mobile drawer style count')
  assert.ok(!/\b(?:src|href)\s*=\s*["'](?!#|data:)/i.test(mobileEnvelope), 'Mobile drawer references an external resource')
  assert.ok(!/@import\s/.test(mobileEnvelope), 'Mobile drawer imports an external stylesheet')
  assert.match(mobileDrawer, /role="dialog"/, 'Mobile drawer lacks dialog semantics')
  assert.match(mobileDrawer, /safe-area-inset-bottom/, 'Mobile drawer lacks safe-area padding')
