...
async function main() {
  ...
  const result = await build({
    ...
    build: {
      ...
      minify: true,
      cssCodeSplit: false,
      lib: { entry: resolve(directory, 'directory-app.tsx'), name: 'MerdeckDirectoryPrototype', formats: ['iife'] },
    },
  })
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:; font-src data:; connect-src 'none'; base-uri 'none'; form-action 'none'"><title>Merdeck — Directory navigation</title><link rel="icon" href="data:,"><style>${styles.join('\n').replace(/<\/style/gi, '<\\/style')}</style></head><body><div id="root"></div><script>${bootstrap}</script></body></html>\n`
  await writeFile(resolve(directory, 'Directory-navigation.html'), html)
  await mkdir(resolve(root, 'tmp/directory-design'), { recursive: true })
  await writeFile(resolve(root, 'tmp/directory-design/build-resources.json'), `${JSON.stringify({ bytes: Buffer.byteLength(html), scripts: scripts.length, styles: styles.length, externalAssets: 0, toolchain: fromWeb.resolve('vite') }, null, 2)}\n`)
  process.stdout.write(`Built Directory-navigation.html (${Buffer.byteLength(html)} bytes; scripts and styles embedded)\n`)
}
