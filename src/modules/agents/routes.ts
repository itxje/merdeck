...
function owner(session: ReturnType<typeof requireSession>, origin: string): AgentPrincipal {
  if (!session)
    throw new AppError('forbidden')
  return { id: session.id, origin, expiresAt: session.expiresAt }
}
...
export function agentRoutes(diagrams: DiagramService, sessions: Sessions | undefined, agents: AgentManager) {
  const router = new Hono<HttpEnvironment>()

  router.get('/agents/capabilities', async (c) => {
    queryInput(new URL(c.req.url), z.strictObject({}))
    owner(requireSession(sessions, c.req.raw, c.get('origin')), c.get('origin'))
  ...
  router.post('/agents/conversations', async (c) => {
    ...
    return c.json({ success: true as const, data: await agents.create(request.provider, request.model, owner(session, c.get('origin'))) })
  })
  ...
  router.post('/agents/conversations/:id/turns', async (c) => {
    ...
    return c.json({ success: true as const, data: await agents.startTurn(id, owner(session, c.get('origin')), request.prompt) })
  })
  ...
  router.post('/agents/conversations/:id/cancel', async (c) => {
    ...
    return c.json({ success: true as const, data: await agents.cancel(validateId(c.req.param('id')), owner(session, c.get('origin'))) })
  })

  router.get('/agents/conversations/:id/events', (c) => {
    const session = requireSession(sessions, c.req.raw, c.get('origin'))
    const principal = owner(session, c.get('origin'))
