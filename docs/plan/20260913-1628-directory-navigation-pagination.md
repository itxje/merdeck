...
Add one metadata-only directory page endpoint, one metadata revision endpoint, and one best-effort explicit cursor-close endpoint. Keep the existing file document/revision/save and entry endpoints. Use a held nonrecursive directory stream, a bounded in-process continuation table, and forward-only, single-use opaque cursors. Do not add a dependency, database, filesystem index, watcher, offset scan, native helper, or whole-directory sort. Use one directory/page request at a time in the UI and retain a bounded page window.
...
| Dirents per page | Fixed 1,024 | Count every actual read, including excluded entries; no extra EOF probe after the boundary. |
...
| Runtime directory prefetch | `bufferSize: 1`, `recursive: false` | One explicit `Dir.read` at a time; no `readdir` array. Runtime internal syscall buffering is not a filesystem snapshot. |
...
| Stream memory | No previous pages; one pending entry plus at most 32 KiB serialized metadata per stream | Hard resource admission; no whole-directory seen-name set. |
...
1. Validate syntax and authorization. Enforce operation capacity. Validate the configured root and every actual ancestor through fresh `O_NOFOLLOW`/`O_DIRECTORY` opens and existing procfs anchors. A new stream opens an owned target anchor and `opendir` from that anchor, confirms matching identity, and records the fingerprint. Close temporary ancestors in `finally`.
...
Use explicit `Dir.read()` and `Dir.close()`, rather than breaking an async iterator and expecting to resume it. The [official filesystem API](https://nodejs.org/api/fs.html#class-fsdir) documents unordered enumeration, possible effects of additions/removals during iteration, and automatic close when an async iterator exits. This motivates the design; actual behavior on pinned Bun 1.4.2 must be tested, not inferred from those API docs.
