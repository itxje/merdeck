...
- Deployment and live evidence (2026-09-12): the hosted instance was relaunched from the main checkout with a build of the sources committed as `c964a27`. In a real browser session its status bar read `Open access | External edits refresh automatically | Merdeck development` at 1440 px and `Open access | Merdeck development` at 390 px, matching a service started from source, and its session reported the same version, with no page or console errors.
