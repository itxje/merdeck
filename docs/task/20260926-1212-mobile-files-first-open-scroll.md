...
- Focused verification: six related Chromium browser cases pass, including the new first-open case. The same first-open focus/geometry case passes on Linux WebKit, which cannot emulate native iOS finger scrolling. Frontend build, lint (two existing warnings) and typecheck pass. Full repository gate and affected-device confirmation remain pending.
