...
- reopen: Owner authorized two browser-test-only repairs after release attempts; preserve successful native and failed publication evidence.

## Browser acceptance timing repair findings and proposal

The owner authorized two test-only corrections after successful candidate native run 34777151314 and integrated main run 34777678520. The immutable v0.9.0 tag identifies `fca683bac16cba466892c3da16b795df94d82146`, but publication failed: first the F2 rename test dispatched before refreshed listing/action readiness; the unchanged retry passed executable checks then failed a bundle assertion whose broad file-name locator matched both the dirty file row and its action button. Release v0.9.0 is not published; v0.9.1 validation/publication remains pending.

Narrowly select the actual welcome file row in both directory assertions and wait for completed listing. In the mutation scenario, synchronize refresh completion and the concrete folder action/focus before dispatching F2 to the row; retain all keyboard, draft, directory and unexpected-error assertions. Reproduce the original failures or deterministic delayed responses, then run only these scenarios against unchanged executable and bundle artifacts. Production code, dependencies and harnesses are read-only. Preserve all prior evidence and use the same task and repair commit.

## Browser test repair candidate and evidence

Both welcome-file assertions now wait for the listing to settle and use an anchored accessible name that permits the dirty suffix without matching the actions button. The F2 scenario deliberately holds one real refresh response, observes loading and disabled mutation controls, releases it, waits for response completion and settled listing/action readiness, then focuses the concrete folder row and dispatches F2 through that locator. All original draft bytes, mutations, new-directory checks, keyboard contract and global unexpected-error auditing remain intact. No retry/timeout increase or production change was made.

Focused evidence under ignored `tmp/browser-timing/`:

- The unmodified tests happened to pass the initial local executable run (`red-executable.log`, exit 0); this is not RED evidence. Original hosted failures remain in `/tmp/merdeck-directory-release/failed.log` and `/tmp/merdeck-directory-release-retry/failed.log`.
- Deterministic RED waits for the actions button before exercising the original ambiguous locator, and schedules the original F2 dispatch while a real refresh response is held after readiness has changed. Both scenarios failed on both the actual executable and bundle (`delayed-red-executable.log`, `delayed-red-bundle.log`, exit 1 each), respectively with strict locator ambiguity and missing rename dialog. The isolated test variants and error contexts are retained. An initial ignored bundle driver's argument error is separately retained as `bundle-driver-initial.log`, not counted as a test failure.
- GREEN: the same two complete scenarios passed against the same unchanged executable and bundle (`green-executable.log`, `green-bundle.log`, exit 0 each), with zero unexpected browser errors and cleanup true. All 94 assets were verified; executable isolation/trace checks passed and the bundle extraction inventory remained unchanged. No full browser suite or local aggregate ran.
- The executable manifest identifies `17e50a4ac4d115d35b9f7ee81e767e31feae6a2b`, Bun 1.4.2, Linux arm64. Its application source is unchanged through the required parent merge (`application-diff.txt` is empty). A fixture bundle was packaged once from that unchanged application and existing frontend assets before RED, with manifest commit `7ef393a2e2beb714a56df98004aa5486cb3382d0`. Both artifacts share all 94 frontend asset hashes and were reused unchanged for GREEN (`application-manifests.json`, `artifact-checksums.txt`). These local fixture artifacts are not a published release.
- Test-file ESLint, frontend TypeScript checking and `git diff --check` passed (`lint.log`, `types.log`). Only a whitespace formatting correction followed browser validation; no executable behavior changed afterward.
- Shared PMA review policy and the TypeScript frontend pack were applied to the complete two-file test diff: no high-confidence findings at any severity, verdict PASS. Exact file-row identity, request completion, focus/F2 dispatch, held-response cleanup, unchanged audits and artifact provenance were checked.
