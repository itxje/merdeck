...
**2026-09-12 explorer file types:** the instance in the same `domain` window was stopped and relaunched from the main checkout with a build of the sources committed as `2195c40`, which lets the explorer list only the chosen file types, keeping the `merdeck` nsl route, demo root and open access. Its service processes exited and its route disappeared before the relaunch. After the restart `https://merdeck.example.test/api/health` returned 200 with service `merdeck`, and `/` and `/favicon.svg` returned 200. A real browser session then listed all four files, chose diagram files and Markdown files in turn, confirmed the rows and the file count for each choice, kept the choice across a reload, and saw the same choice in the narrow file drawer; see [FILE-003](task/FILE-003.md).
