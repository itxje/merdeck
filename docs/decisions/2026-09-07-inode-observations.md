...
## Content identity model (2026-09-12)

The measurement above led to [PLAN-020](../plan/PLAN-020.md), approved by the owner and implemented as [STORAGE-002](../task/STORAGE-002.md): the save protocol no longer assumes that every admitted storage keeps an inode number stable. The measured mount now selects an identity model. Overlay and ext4 keep the `stable` model with its device and inode comparison, byte for byte as before. `virtiofs` is admitted under a `content` model that compares device, size, both timestamps and the complete-file hash, proves staged bytes by their own hash, reads the published name back after the rename, and proves an entry move through size, modification time and the link count, because a hard link raises the link count and therefore moves the change time.

This trades one distinction for the storage that could not be written to at all: an external replacement whose bytes and both timestamps are identical no longer conflicts on the content model. Content that differs is still refused by the hash, a timestamp difference is still refused, and the strict model is unchanged where it works. Directory and root identity were measured stable on that storage across 2,000 checks before the change, and hard links reported identical inodes and a link count of two in 200 of 200 cases. After the change, the file, storage, API and acceptance tests ran twenty times over with fixtures on that storage: 1,060 passed and 0 failed, against 5 failures in 80 runs before it, and the browser suite passed 42 of 42 against a service rooted there.
